...
type IconProps = SVGProps<SVGSVGElement> & {
  size?: number;
  /** When true, the icon renders as a filled silhouette instead of an
   *  outlined stroke. Used by the bottom tab bar for active states. */
  filled?: boolean;
};
...
/* ─────────────────────────── Profile ─────────────────────────── */
export function ProfileIcon({ size = 24, filled, ...rest }: IconProps) {
  if (filled) {
    return (
      <svg {...baseSvgProps(size)} {...rest}>
        <circle cx="12" cy="8.25" r="3.75" fill="currentColor" />
        <path
          d="M4.5 20.25c0-3.728 3.358-6.75 7.5-6.75s7.5 3.022 7.5 6.75v.5H4.5v-.5Z"
          fill="currentColor"
        />
      </svg>
    );
...
export function HomeIcon({ size = 24, filled, ...rest }: IconProps) {
  if (filled) {
    return (
      <svg {...baseSvgProps(size)} {...rest}>
        <path
          d="M3.5 10.75 12 4l8.5 6.75V20a.75.75 0 0 1-.75.75H14.25v-5.5h-4.5v5.5H4.25A.75.75 0 0 1 3.5 20v-9.25Z"
          fill="currentColor"
        />
      </svg>
    );
...
export function CalendarIcon({ size = 24, filled, ...rest }: IconProps) {
  if (filled) {
    return (
      <svg {...baseSvgProps(size)} {...rest}>
        {/* solid calendar block with header strip + cutout dots */}
        <rect x="3.5" y="5.5" width="17" height="15" rx="2.5" fill="currentColor" />
        <path
          ...
          strokeLinecap="round"
        />
        {/* knockout dots reading as date pips */}
        <circle cx="8.25" cy="14" r="0.9" fill="#FFFFFF" />
        <circle cx="12" cy="14" r="0.9" fill="#FFFFFF" />
        <circle cx="15.75" cy="14" r="0.9" fill="#FFFFFF" />
        <circle cx="8.25" cy="17.25" r="0.9" fill="#FFFFFF" />
        <circle cx="12" cy="17.25" r="0.9" fill="#FFFFFF" />
      </svg>
    );
  }
  return (
...
export function InsightsIcon({ size = 24, filled, ...rest }: IconProps) {
  if (filled) {
    return (
      <svg {...baseSvgProps(size)} {...rest}>
        {/* filled bar-chart silhouette — three solid bars sitting on a baseline */}
        <rect x="4" y="13" width="3.5" height="8" rx="1" fill="currentColor" />
        <rect x="10.25" y="9" width="3.5" height="12" rx="1" fill="currentColor" />
        <rect x="16.5" y="5" width="3.5" height="16" rx="1" fill="currentColor" />
      </svg>
    );
  }
  return (
